...
def evaluate(classifier, data_loader, criterion_cls, vis, writer, iteration, device):
    ...
    return overall_mean_loss, acc, total, total_test_time
...
def main():
    ...
    assert params.valid_ratio > 0, "Zero validation ratio is not allowed "
    split = int(np.floor((1.0 - params.valid_ratio) * num_train))
    ...
    valid_dataloader = torch.utils.data.DataLoader(train_set, batch_size=params.valid_batch_size,
                                                   sampler=valid_sampler, num_workers=params.n_workers)
    ...
    start_epoch = 0
    max_train_acc = max_valid_acc = 0
    min_train_loss = min_valid_loss = np.inf
    max_train_acc_epoch = max_valid_acc_epoch = 0
    min_train_loss_epoch = min_valid_loss_epoch = 0
    ...
    if params.load_weights:
        ...
        if not matching_ckpts:
            ...
            min_train_loss_epoch = min_valid_loss_epoch = max_train_acc_epoch = max_valid_acc_epoch = chkpt['epoch']
    ...
    if params.load_weights != 1:
        ...
        for epoch in range(start_epoch, params.n_epochs):
            ...
            if epoch % params.valid_gap == 0:

                valid_loss, valid_acc, _, valid_time = evaluate(
                    classifier, valid_dataloader, criterion, params.vis, writer, epoch, device)
                fps = n_valid / valid_time
                print('validation time: {:.4f} sec. with {:d} images ({:.4f} images / sec)'.format(
                    valid_time, n_valid, fps))
                ...
                status_df['loss']['validation'] = valid_loss
                status_df['acc']['validation'] = valid_acc
                status_df['min_loss (epoch)']['validation'] = '{:.3f} ({:d})'.format(min_valid_loss,
                                                                                     min_valid_loss_epoch)
                status_df['max_acc (epoch)']['validation'] = '{:.3f} ({:d})'.format(max_valid_acc,
                                                                                    max_valid_acc_epoch)
    ...
    _, test_acc, n_test, test_time = evaluate(
